...
class SimulatorBase(ShowBase):
    def __init__(self):
        ...
        props.setTitle("FF14 P4 Simulator beta - Gamepad")
        ...
        # 右スティック = カメラ回転
        self.gamepad = None
        self.pad_deadzone = 0.18
        self.pad_camera_speed_x = 120.0
        self.pad_camera_speed_y = 80.0
        self.camera_pitch_min = -10.0
        self.camera_pitch_max = 55.0
        self.setup_gamepad()
        ...
        # --- 入力 ---
        # パッド版では移動・カメラ操作をゲームパッドに一本化する。
        # GUIボタン自体は従来どおりマウスでも操作可能。
    ...
    def setup_gamepad(self):
        """起動時に接続済みのゲームパッドを1台取得する。"""
        for device in self.devices.getDevices():
            if device.device_class.name == "gamepad":
                self.gamepad = device
                self.attachInputDevice(device, prefix="gamepad")
                print(f"Gamepad connected: {device.name}")
                return

        print("Gamepad not found. Please connect a controller.")

    def apply_deadzone(self, value: float) -> float:
        """スティック中央付近の微小入力を無視する。"""
        if abs(value) < self.pad_deadzone:
            return 0.0
        return value

    def get_axis_value(self, axis):
        """接続中ゲームパッドの指定Axis値を返す。"""
        if self.gamepad is None:
            return 0.0

        axis_state = self.gamepad.findAxis(axis)
        if axis_state is None:
            return 0.0

        return self.apply_deadzone(axis_state.value)
    ...
    def update_camera(self, dt: float):
        ...
        横 = right_x / 縦 = right_y
        """
        rx = self.get_axis_value(InputDevice.Axis.right_x)
        ry = self.get_axis_value(InputDevice.Axis.right_y)

        self.camera_yaw += rx * self.pad_camera_speed_x * dt
        self.camera_pitch -= ry * self.pad_camera_speed_y * dt
        self.camera_pitch = max(
            self.camera_pitch_min,
            min(self.camera_pitch_max, self.camera_pitch)
        )
    ...
    def update_player(self, dt: float):
        ...
        カメラ基準でプレイヤーを移動する。
        """
        lx = self.get_axis_value(InputDevice.Axis.left_x)
        ly = self.get_axis_value(InputDevice.Axis.left_y)
        ...
        if self.player_is_moving:
            ...
            # P4の視線判定などで使うプレイヤー向きも、移動方向へ更新する。
